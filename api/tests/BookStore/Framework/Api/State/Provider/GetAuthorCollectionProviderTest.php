...
use App\BookStore\Framework\Api\State\Provider\GetAuthorCollectionProvider;
 ...
 * @internal
 */
class GetAuthorCollectionProviderTest extends TestCase
{
    use ProphecyTrait;
    ...
    public function test_it_launchs_the_correct_command()
    {
        ...
        $commandBus->handle(Argument::type(SearchAuthor::class))
            ->willReturn([])
            ->shouldBeCalledOnce()
        ;

        $provider = new GetAuthorCollectionProvider($commandBus->reveal(), $repository->reveal());
