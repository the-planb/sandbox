...
use App\BookStore\Framework\Api\State\Provider\GetTagCollectionProvider;
 ...
 * @internal
 */
class GetTagCollectionProviderTest extends TestCase
{
    use ProphecyTrait;
    ...
    public function test_it_launchs_the_correct_command()
    {
        ...
        $commandBus->handle(Argument::type(SearchTag::class))
            ->willReturn([])
            ->shouldBeCalledOnce()
        ;

        $provider = new GetTagCollectionProvider($commandBus->reveal(), $repository->reveal());
